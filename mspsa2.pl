...
#
# MSPSA Tuner is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
#
# MSPSA Tuner is distributed in the hope that it will be useful,
...
sub run_spsa
{
    ...
    while(1)
    { 
        # MSPSA coefficients indexed by variable.
...

        {
             ...
             foreach $row (@variables)
             {
                 ...
                 if ($iter > $A + 1 && ($iter - 1) % $A == 0){
                      ...
                      $var_a{$name} = ($var_prog1{$name} > 0.001 ? min(5 * $alpha, max($alpha, $var_a{$name} * sqrt($var_prog2{$name} / $var_prog1{$name}))): $var_a{$name});
                      $var_c{$name} = ($var_prog1{$name} > 0.001 ? min(5 * $gamma, max($gamma, $var_c{$name} * sqrt($var_prog2{$name} / $var_prog1{$name}))): $var_c{$name});
...
READ:      while($line = engine_readline($Curr_Reader)) 
           ...
           if ($engine_to_move == 1) 
           {
               $eng1_time -= $elapsed * 1000 - $inc_time - (++$move_counter[1] % 40 == 0 ? $base_time : 0);
           }
           else
           {
               $eng2_time -= $elapsed * 1000 - $inc_time - (++$move_counter[2] % 40 == 0 ? $base_time : 0);
           }
